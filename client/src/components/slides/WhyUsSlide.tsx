...
export default function WhyUsSlide() {
  ...
  return (
    <div ref={ref} className="snap-section relative grain-overlay" style={{ background: '#0a0f1a' }}>
      <div className="relative z-10 h-full flex flex-col lg:flex-row items-center px-8 md:px-16 lg:px-24 py-16 gap-8 lg:gap-16">
          ...
          <AnimatedSection animation="fade-in" delay={0.2} className="w-full">
            <div className="relative rounded-xl overflow-hidden" style={{ maxHeight: '65vh' }}>
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6 flex gap-6">
                {[
                  { icon: <Building2 className="w-4 h-4" />, value: 4, label: '已落成空间', suffix: '个' },
                  { icon: <Users className="w-4 h-4" />, value: 2, label: '建设中', suffix: '个' },
                  { icon: <Network className="w-4 h-4" />, value: 5, label: '覆盖城市', suffix: '座' },
                ].map((s, i) => (
                  <div key={i} className="flex items-center gap-2">
                    <div className="text-[#d4a853]">{s.icon}</div>
                    <div>
                      <div className="text-xl font-bold text-[#d4a853]" style={{ fontFamily: "'Space Grotesk', monospace" }}>
                        <Counter value={s.value} isActive={isInView} />{s.suffix}
                      </div>
                      <div className="text-[#e8e4df]/50 text-xs" style={{ fontFamily: "'Noto Sans SC', sans-serif" }}>{s.label}</div>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="lg:w-1/2 flex flex-col justify-center">
          ...
          <AnimatedSection animation="slide-up" delay={0.35}>
            <p className="text-[#e8e4df]/65 text-base leading-relaxed mb-6" style={{ fontFamily: "'Noto Sans SC', sans-serif" }}>
              我们是极少数同时具备<strong className="text-[#d4a853]">乡村实地运营、社区组织、产品化和智能体落地能力</strong>的团队。过去几年，已落成 4 个大型线下社区，另有 2 个正在建设中。
            </p>
          </AnimatedSection>

          <AnimatedSection animation="slide-up" delay={0.5}>
            <p className="text-[#e8e4df]/65 text-base leading-relaxed mb-8" style={{ fontFamily: "'Noto Sans SC', sans-serif" }}>
          ...
          <AnimatedSection animation="slide-up" delay={0.65}>
            <div className="border-l-2 border-[#d4a853]/30 pl-6">
              <p className="text-[#d4a853] text-lg italic" style={{ fontFamily: "'Noto Serif SC', serif" }}>
                别人做的是软件，我们做的是
                <br />
                "AI 系统 + 线下节点 + 流动人才网络"
                <br />
