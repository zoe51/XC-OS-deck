    ...
    <div ref={ref} className="snap-section relative grain-overlay" style={{ background: '#0a0f1a' }}>
      ...
      <div className="relative z-10 h-full flex flex-col justify-center px-8 md:px-16 lg:px-24 py-16">
        ...
        <AnimatedSection animation="slide-up" delay={0.2}>
          ...
          >
            社会<span style={{ background: 'linear-gradient(135deg, #e8a87c, #d4a853)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>效益</span>
          </h2>
        </AnimatedSection>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl">
          {impacts.map((impact, i) => (
            <AnimatedSection key={i} animation="slide-up" delay={0.35 + i * 0.15}>
              <div className="glass-card rounded-lg p-6 h-full group hover:translate-y-[-4px] transition-all duration-500">
                <div
                  ...
                  style={{ backgroundColor: `${impact.color}12`, color: impact.color }}
                >
                  {impact.icon}
                </div>
                <h3
                  className="text-lg font-semibold mb-3"
                  style={{ color: impact.color, fontFamily: "'Noto Sans SC', sans-serif" }}
                >
                  {impact.title}
                </h3>
                {impact.stat && (
                  <div className="text-3xl font-bold mb-2" style={{ color: impact.color, fontFamily: "'Space Grotesk', monospace" }}>
