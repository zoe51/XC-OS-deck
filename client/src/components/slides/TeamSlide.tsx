...
export default function TeamSlide() {
  return (
    <div className="snap-section relative grain-overlay" style={{ background: 'linear-gradient(180deg, #0a0f1a 0%, #0d1320 100%)' }}>
      {/* Decorative */}
      <div className="absolute top-1/3 left-1/4 w-64 h-64 rounded-full blur-[100px] opacity-8"
      ...
      <div className="relative z-10 h-full flex flex-col justify-center px-8 md:px-16 lg:px-24 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
          {team.map((member, i) => (
            <AnimatedSection key={i} animation="slide-up" delay={0.3 + i * 0.12}>
              <div className="glass-card rounded-lg p-6 h-full group hover:translate-y-[-4px] transition-all duration-500">
                ...
                >
                  <span className="text-lg font-bold" style={{ color: member.color, fontFamily: "'Space Grotesk', monospace" }}>
                    {member.initials}
                  </span>
                </div>

                {/* Name & Role */}
                <h3 className="text-xl font-bold mb-1" style={{ color: '#e8e4df', fontFamily: "'Noto Serif SC', serif" }}>
                  {member.name}
                </h3>
                <p className="text-sm font-medium mb-4" style={{ color: member.color, fontFamily: "'Noto Sans SC', sans-serif" }}>
                  {member.role}
                </p>
                ...
                <div className="space-y-2">
                  {member.bg.map((item, j) => (
                    <p key={j} className="text-[#e8e4df]/45 text-xs leading-relaxed flex items-start gap-2" style={{ fontFamily: "'Noto Sans SC', sans-serif" }}>
                      <span className="w-1 h-1 rounded-full mt-1.5 shrink-0" style={{ backgroundColor: `${member.color}50` }} />
                      {item}
                    </p>
                  ))}
                </div>
