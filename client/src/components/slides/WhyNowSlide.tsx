...
function StatNumber({ value, suffix, isActive }: { value: number; suffix: string; isActive: boolean }) {
...
export default function WhyNowSlide() {
  ...
  return (
    <div ref={ref} className="snap-section relative grain-overlay" style={{ background: 'linear-gradient(135deg, #0a0f1a 0%, #0c1220 50%, #0a0f1a 100%)' }}>
      ...
      <div className="relative z-10 h-full flex flex-col justify-center px-8 md:px-16 lg:px-24 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {reasons.map((r, i) => (
            <AnimatedSection key={i} animation="slide-up" delay={0.35 + i * 0.12}>
              <div className="glass-card rounded-lg p-6 h-full group hover:translate-y-[-4px] transition-all duration-500">
                ...
                <h3 className="text-base font-semibold mb-2" style={{ color: '#e8e4df', fontFamily: "'Noto Sans SC', sans-serif" }}>
                  {r.title}
                </h3>
